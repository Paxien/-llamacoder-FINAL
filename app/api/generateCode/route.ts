...
import { z } from "zod";
import { createProvider, parseModelString } from "@/utils/providers/factory";
...
export async function POST(req: Request) {
  try {
    ...
    let { model: modelString, messages, shadcn } = result.data;
    ...
    try {
      ...
      const stream = await aiProvider.createChatCompletion({
        model,
        messages: [
          {
            role: "system",
