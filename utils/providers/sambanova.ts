...
export class SambaNovaProvider implements AIProvider {
  ...
  constructor() {
    if (!process.env.SAMBA_NOVA_API_KEY) {
      throw new Error('SAMBA_NOVA_API_KEY is required for SambaNova provider');
    }

    this.client = new OpenAI({
      apiKey: process.env.SAMBA_NOVA_API_KEY,
      baseURL: 'https://api.sambanova.ai/api/v1/completions'
    });
  }
  ...
  async createChatCompletion(params: ChatCompletionParams): Promise<ReadableStream> {
    try {
      ...
      return new ReadableStream({
        async start(controller) {
          try {
            ...
            console.error('Error in SambaNova stream:', error);
      ...
      console.error('Error in SambaNova provider:', error);
      throw error;
    }
  }

  private mapModel(model: string): string {
    // Map from our internal model names to SambaNova model names
    const modelMap: Record<string, string> = {
      'sambanova-1': 'sambanova-1',
      'sambanova-2': 'sambanova-2',
      'sambanova-3': 'sambanova-3',
    };
